...
function load_routes () {
	...
	foreach ($mod_installed as $module) {
		Route::group(['middleware' => [function (Request $req, Closure $next) use ($module) {
			// Validar se módulo obriga uso de SSL-HTTPS
			if ($module->force_ssl && !$req->secure())
				return redirect()->secure($req->getRequestUri());

			// Caso passar, continuar com inicialização
			return $next($req);
		}]], function () use ($module, &$mod_preloaded, &$mod_installed) {
