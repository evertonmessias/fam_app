...
class RDStationAPI {
  ...
  public function setProxy ($proxy = null) { $this->proxy = $proxy; }
  ...
  public function __construct($privateToken=NULL, $token=NULL){
	...
	**/
  public function getURL($type='generic', $apiVersion='1.2'){
  ...
  public function request($method="POST", $url, $data=array()){
    ...
    if (is_null($this->proxy)) {
      ...
      $fp = fsockopen($this->proxy['host'],
                    isset($this->proxy['port'])?$this->proxy['port']:8080,
                    $errno, $errstr, 30);

      $out = $method." ".$URLParts['scheme'].'://'.$URLParts['host'].$URLParts['path']." HTTP/1.1\r\n";
      $out .= "Host:". $URLParts['host'] . "\r\n";

      if (isset($this->proxy['user']) && isset($this->proxy['pass']))
        $out .= "Proxy-Authorization: Basic " . base64_encode($this->proxy['user'] . ":" . $this->proxy['pass']) . "\r\n\r\n";
    }
