...
Route::get('/', function () use ($module) {
	...
	return view ('TOP3::index', $options);
});
...
Route::post('/quiz', function (Request $req) use ($module) {
	...
	return view ('TOP3::pergunta', $options);
});
...
Route::post('/resultados', function (Request $req) {
    ...
    $tempCampo = json_decode($campos);

    // Calcular pontuação
    $campos = array_flatten(json_decode($campos)->areas);
